...
function blankVec() { return new Array(DIM_KEYS.length).fill(0); }
...
function scoreFromAnswers(answerMap, questions) {
  ...
  for (const [qPrefix, aPrefix] of Object.entries(answerMap)) {
    ...
    if (!q) { unmatched.push({ qPrefix, why: 'no question matched prefix' }); continue; }
    const a = q.answers.find(a => a.text.startsWith(aPrefix));
    if (!a) { unmatched.push({ qPrefix, why: `no answer in matched question starts with "${aPrefix}"` }); continue; }
...
const CANONICAL_PERSONAS = [
  {
    name: 'The Stoic',
    description: 'Marcus Aurelius reader who actually practices premortems. Focused on what they control, accepting of what they can\'t. Quiet.',
    expected: ['keel'],
    vector: vecFromShorthand({ TV:7, VA:6, WP:2, TR:8, TE:6, RT:6, MR:3, SR:5, CE:6, SS:5, PO:10, TD:5, AT:8, ES:3, UI:7, SI:3 }),
  },
  {
    name: 'The Existentialist',
    description: 'Camus reader. Knows the absurd is real, refuses both suicide and philosophical retreat. Acts with full freedom.',
    expected: ['pilgrim'],
    vector: vecFromShorthand({ TV:8, VA:5, WP:7, TR:5, TE:7, RT:2, MR:3, SR:4, CE:4, SS:9, PO:6, TD:4, AT:4, ES:6, UI:4, SI:5 }),
  },
  {
    name: 'The Systematic Rationalist',
    description: 'Plato + Kant reader. Reality has a rational structure; reason can grasp it; the universal is what matters.',
    expected: ['lighthouse'],
    vector: vecFromShorthand({ TV:4, VA:5, WP:4, TR:10, TE:3, RT:3, MR:6, SR:2, CE:5, SS:4, PO:5, TD:9, AT:6, ES:2, UI:9, SI:3 }),
  },
  {
    name: 'The Patient Mapper',
    description: 'Spinoza + Parfit reader. Build the system. Trace how everything connects. Don\'t skip steps.',
    expected: ['cartographer'],
    vector: vecFromShorthand({ TV:5, VA:5, WP:4, TR:9, TE:6, RT:5, MR:4, SR:4, CE:5, SS:4, PO:8, TD:9, AT:5, ES:5, UI:7, SI:4 }),
  },
  {
    name: 'The Sceptic',
    description: 'Hume + Sextus Empiricus reader. Suspend judgment. Most philosophical certainties dissolve under pressure.',
    expected: ['touchstone'],
    vector: vecFromShorthand({ TV:6, VA:5, WP:3, TR:6, TE:7, RT:4, MR:3, SR:9, CE:5, SS:6, PO:6, TD:6, AT:4, ES:6, UI:4, SI:6 }),
  },
  {
    name: 'The Iconoclast',
    description: 'Nietzsche + Stirner reader. The inherited values are dead. Sovereign self against the herd. Break what doesn\'t serve.',
    expected: ['hammer'],
    vector: vecFromShorthand({ TV:5, VA:7, WP:9, TR:6, TE:6, RT:1, MR:2, SR:5, CE:2, SS:10, PO:6, TD:5, AT:4, ES:7, UI:3, SI:4 }),
  },
  {
    name: 'The Mystic',
    description: 'Rumi + Pseudo-Dionysius reader. The deepest truths are apophatic. Reason runs out before reality does.',
    expected: ['threshold'],
    vector: vecFromShorthand({ TV:7, VA:7, WP:3, TR:4, TE:5, RT:7, MR:10, SR:5, CE:6, SS:4, PO:5, TD:8, AT:7, ES:4, UI:7, SI:7 }),
  },
  {
    name: 'The Reformist Liberal',
    description: 'Mill + Wollstonecraft reader. Build forward through institutions. Reason + liberty + mutual sympathy.',
    expected: ['forge', 'garden'],
    vector: vecFromShorthand({ TV:4, VA:7, WP:6, TR:8, TE:7, RT:3, MR:2, SR:5, CE:6, SS:7, PO:8, TD:6, AT:3, ES:6, UI:8, SI:3 }),
  },
  {
    name: 'The Tradition-Keeper',
    description: 'Confucius + Augustine reader. The practices that survived weren\'t lucky. Inherit and pass on.',
    expected: ['hearth'],
    vector: vecFromShorthand({ TV:6, VA:6, WP:2, TR:6, TE:6, RT:9, MR:5, SR:4, CE:9, SS:2, PO:7, TD:5, AT:5, ES:5, UI:6, SI:3 }),
  },
  {
    name: 'The Epicurean Gardener',
    description: 'Epicurus + Montaigne reader. Cultivate the small good things. Friendship, simple pleasures, freedom from anxiety.',
    expected: ['garden'],
    vector: vecFromShorthand({ TV:3, VA:9, WP:3, TR:6, TE:8, RT:5, MR:2, SR:6, CE:7, SS:6, PO:9, TD:5, AT:3, ES:8, UI:5, SI:4 }),
  },
];
...
const EDGE_PERSONAS = [
  {
    ...
    description: 'Stoic reader who also walks alone with the question — Marcus Aurelius meets Camus. Should be Keel or Pilgrim depending on which way the dimensions tip.',
    expected: ['keel', 'pilgrim'],
    vector: vecFromShorthand({ TV:7, VA:5, WP:4, TR:7, TE:7, RT:4, MR:3, SR:5, CE:5, SS:7, PO:8, TD:5, AT:6, ES:5, UI:5, SI:4 }),
  },
  {
    name: 'The Engaged Cartographer',
    description: 'Builds the system but uses it in the world — Spinoza-style monism turned practical. Cartographer or Forge.',
    expected: ['cartographer', 'forge'],
    vector: vecFromShorthand({ TV:5, VA:6, WP:6, TR:9, TE:6, RT:4, MR:3, SR:4, CE:6, SS:5, PO:9, TD:8, AT:4, ES:5, UI:7, SI:4 }),
  },
  {
    name: 'The Quiet Mystic-Sceptic',
    description: 'Knows apophatic mysticism, suspends ordinary judgment, neither claims nor denies. Threshold or Touchstone.',
    expected: ['threshold', 'touchstone'],
    vector: vecFromShorthand({ TV:6, VA:6, WP:3, TR:5, TE:6, RT:6, MR:8, SR:8, CE:5, SS:5, PO:5, TD:7, AT:6, ES:4, UI:6, SI:7 }),
  },
  {
    name: 'The Reformist Tradition-Keeper',
    description: 'Reveres inherited practice, would still adjust it — Burkean-Whig. Hearth or Forge.',
    expected: ['hearth', 'forge'],
    vector: vecFromShorthand({ TV:5, VA:6, WP:5, TR:7, TE:7, RT:8, MR:4, SR:5, CE:8, SS:5, PO:8, TD:6, AT:4, ES:5, UI:7, SI:3 }),
  },
  {
    name: 'The Lighthouse-Touchstone Borderline',
    description: 'Wants reasoned universal truth — but holds doubt as a discipline. Wittgenstein late + Kant\'s critical edge. Lighthouse or Touchstone.',
    expected: ['lighthouse', 'touchstone'],
    vector: vecFromShorthand({ TV:6, VA:5, WP:4, TR:8, TE:5, RT:4, MR:5, SR:8, CE:4, SS:5, PO:5, TD:8, AT:5, ES:4, UI:7, SI:6 }),
  },
];
...
const PARADOX_PERSONAS = [
  {
    ...
    description: 'Reveres inherited institutions AND insists moral authority is internal. Burke + Stirner. The high-RT, high-SS quadrant the 10-archetype model deliberately doesn\'t cover (see /methodology §Open Questions). Expected: any plausible archetype with a low margin so the result UI surfaces tension.',
    expectLowMargin: true,
    vector: vecFromShorthand({ TV:5, VA:6, WP:6, TR:7, TE:6, RT:8, MR:4, SR:5, CE:5, SS:8, PO:7, TD:6, AT:4, ES:5, UI:6, SI:4 }),
  },
  {
    name: 'The Mystical Empiricist',
    description: 'Trusts bodily experience AND reaches for the apophatic. James + Rumi. Crosses Garden and Threshold.',
    expectLowMargin: true,
    vector: vecFromShorthand({ TV:6, VA:8, WP:4, TR:5, TE:9, RT:5, MR:8, SR:5, CE:6, SS:6, PO:7, TD:6, AT:4, ES:8, UI:5, SI:5 }),
  },
  {
    name: 'The Practical Idealist',
    description: 'Universal moral principles AND practical worldly engagement. Kant\'s ethics + Mill\'s reformism. Crosses Lighthouse and Forge.',
    expectLowMargin: true,
    vector: vecFromShorthand({ TV:4, VA:6, WP:6, TR:9, TE:6, RT:4, MR:3, SR:4, CE:5, SS:5, PO:9, TD:7, AT:4, ES:5, UI:9, SI:3 }),
  },
  {
    name: 'The Solitary Communitarian',
    description: 'Treats the self as embedded in tradition AND insists on inward solitude. Augustine + Kierkegaard. Crosses Hearth and Pilgrim.',
    expectLowMargin: true,
    vector: vecFromShorthand({ TV:7, VA:5, WP:3, TR:5, TE:5, RT:8, MR:7, SR:5, CE:8, SS:6, PO:5, TD:6, AT:6, ES:4, UI:6, SI:5 }),
  },
];
...
async function main() {
  ...
  const archVecs = archetypeTargets.map(a => ({ key: a.key, name: a.name, vec: expandSignature(a.p) }));
  ...
  md.push('');
  md.push(`Quiz parser sanity check: ${results.parserCheck.quick} quick questions, ${results.parserCheck.detailed} detailed questions loaded.`);
  md.push('');
  md.push(`- Canonical (${CANONICAL_PERSONAS.length}): ${results.canonical.length - canonicalFails.length}/${CANONICAL_PERSONAS.length} pass`);
  md.push(`- Edge (${EDGE_PERSONAS.length}): ${results.edge.length - edgeFails.length}/${EDGE_PERSONAS.length} pass`);
  md.push(`- Paradox (${PARADOX_PERSONAS.length}): ${results.paradox.length - paradoxFails.length}/${PARADOX_PERSONAS.length} signal low-margin tension (margin ≤ ${PARADOX_MAX_MARGIN})`);
  md.push('');
  ...
  function renderSection(title, items, kind) {
    ...
    for (const r of items) {
      ...
      md.push('');
      md.push(`Classified: **${r.classification.top.name}** (sim ${r.classification.top.sim.toFixed(3)})`);
      md.push(`Runner-up: ${r.classification.runnerUp.name} (sim ${r.classification.runnerUp.sim.toFixed(3)})`);
      md.push(`Margin: ${r.classification.margin.toFixed(4)}`);
      if (kind === 'paradox') {
        md.push(`Expected: low margin (≤ ${PARADOX_MAX_MARGIN}) to signal tension. ${r.pass ? '✓ tension detected' : '✗ classified too confidently — model thinks this is a single-archetype stance when the persona is intentionally hybrid'}`);
      } else {
  ...
  console.log(`  Canonical: ${CANONICAL_PERSONAS.length - canonicalFails.length}/${CANONICAL_PERSONAS.length}`);
  console.log(`  Edge:      ${EDGE_PERSONAS.length - edgeFails.length}/${EDGE_PERSONAS.length}`);
  console.log(`  Paradox:   ${PARADOX_PERSONAS.length - paradoxFails.length}/${PARADOX_PERSONAS.length} (signal tension)`);
  ...
  if (!QUIET) {
    if (canonicalFails.length) {
      ...
      for (const f of canonicalFails) {
        console.log(`  ${f.persona.name}: classified as ${f.classification.top.name}, expected ${f.persona.expected.join('/')}`);
      }
    }
    if (edgeFails.length) {
      console.log(`\n✗ Edge failures:`);
    ...
    if (paradoxFails.length) {
      ...
      for (const f of paradoxFails) {
        console.log(`  ${f.persona.name}: classified as ${f.classification.top.name} with margin ${f.classification.margin.toFixed(4)} (expected ≤ ${PARADOX_MAX_MARGIN})`);
      }
    }
  }
