...
async function creamTile(logoPng, logoW, logoH, tile, coverage) {
  ...
  const resized = await sharp(logoPng).resize({ height: targetH }).toBuffer();
...
async function main() {
  ...
  await sharp(masterPng)
    ...
    .toFile(path.join(PUB, 'mull-logo.png'));
...
`;
  ...
  const ogLogo = await sharp(path.join(PUB, 'mull-logo.png'))
    .resize({ height: OG_LOGO_H })
    .toBuffer();
