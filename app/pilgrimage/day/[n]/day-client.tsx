...
export default function PilgrimageDayClient({ day, locale }: Props) {
  ...
  const arc = useMemo(
    () => (state && state !== 'loading' ? localizePilgrimageArc(state.archetype, locale) : null),
    [state, locale],
  );
  ...
  function submit() {
    ...
    const next: PilgrimageState = {
      ...state,
      completedDays: Array.from(new Set([...state.completedDays, day])).sort((a, b) => a - b),
      currentDay:
        day === state.currentDay && state.currentDay < 30 ? state.currentDay + 1 : state.currentDay,
    };
    try {
  ...
  return (
    <div className="space-y-5">
      ...
      >
        ...
        <h2 className="mt-2 text-[28px] leading-tight text-ink" style={{ fontFamily: serif }}>
          {dayData.title}
        </h2>
      ...
      {!submitted ? (
        ...
        >
          ...
          {done && (
            <p className="mt-4 text-[13px] italic text-acc-deep" style={{ fontFamily: serif }}>
      ...
      {/* Nav row */}
      <div
        className="flex justify-between text-[11px] tracking-[0.18em]"
