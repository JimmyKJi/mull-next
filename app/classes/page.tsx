...
export default async function ClassesIndexPage() {
  ...
  const [taughtRes, memberRes] = await Promise.all([
    ...
    supabase
      .from('class_members')
      .select(
        'class_id, joined_at, classes ( id, name, term, school_name, is_archived, teacher_user_id )',
      )
      .eq('user_id', user.id)
  ...
  const studentMemberships = (memberRes.data ?? []).filter(
    (m) => m.classes && !m.classes.is_archived,
  );
      ...
      {taught.length > 0 && (
        <section style={{ marginBottom: 36 }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 14 }}>
            {taught.map((c) => (
              <TeacherClassCard key={c.id} c={c} locale={locale} />
            ))}
          </ul>
        </section>
      )}
...
function StudentClassCard({
  ...
  c: NonNullable<StudentClass['classes']>;
  joinedAt: string;
  locale: Locale;
}) {
  return (
        ...
        >
          ...
          >
            {t('cls.card_joined', locale, {
              date: new Date(joinedAt).toLocaleDateString(locale === 'zh' ? 'zh-CN' : 'en-US', {
                month: 'short',
                day: 'numeric',
              }),
            })}
