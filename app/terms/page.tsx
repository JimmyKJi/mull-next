    ...
    <>
      ...
      <div className="space-y-8">
        <PixelWindow title="MULL 是什么" badge="▶ 引言">
          <Prose>
            <p>
              Mull 是一个出于热爱而做的哲学应用：一套对哲学倾向的十六维度刻画、一个原型模型、一道每日困境，以及若干相关的实践（日记、与模拟哲学家的辩论、练习）。它是一件用于思考的工具，而非心理治疗、并非诊断工具，也不能替代任何形式的专业建议。
            </p>
          </Prose>
        </PixelWindow>
        ...
        <PixelWindow title="你的账户" badge="▶ 责任">
          <ul className="space-y-2">
            ...
            <ProseLi>
              你对自己所写的内容负责——困境回应、日记条目、辩论文字、公开资料内容。
            </ProseLi>
            <ProseLi>
        ...
        <PixelWindow title="你不能做什么" badge="▶ 禁区">
          <ul className="space-y-2">
            <ProseLi>
              不要利用 Mull 去骚扰任何人。公开资料以及任何即将推出的社区功能，都不是用来做这种事的工具。
            </ProseLi>
            <ProseLi>
              不要抓取 Mull 的内容（题目、原型文字、哲学家数据库）用于再分发。这些文字皆为手工撰写，模型亦为手工调校；如需复用，请先征询。
            </ProseLi>
            <ProseLi>
              不要试图搞垮我们的服务器——DDoS、漏洞利用尝试、对 AI 接口的自动化滥用。我们已设有速率限制；绕过这些限制即违反本条款。
            </ProseLi>
            <ProseLi>
              不要提交在你所在司法管辖区属于违法、或侵犯他人权利的内容。
            </ProseLi>
          </ul>
        </PixelWindow>
        ...
        <PixelWindow title="我们能做什么" badge="▶ 我们这一方">
          <ul className="space-y-2">
            ...
            <ProseLi>
              随时间推移更改 Mull 的功能、设计与行为方式。现在是 v0.9 版本——一切都会变动。重大变更我们会通过欢迎邮件或横幅来告知。
            </ProseLi>
            <ProseLi>
        ...
        <PixelWindow title="你的内容 · AI · 订阅">
          <Prose>
            <p>
              <strong>你的内容。</strong> 你在 Mull 上所写的一切都归你所有。当你把它发布到公开资料或任何社区版面（在这些功能存在时）上，即表示你授予我们一项非排他性许可，以将其作为 Mull 正常运作的一部分加以展示——这正是选择公开的全部意义所在。你随时可以撤下任何一则公开内容，展示它的许可也随该撤下而终止。
            </p>
            <p>
              <strong>AI 输出。</strong> Mull 在其部分分析中使用了 Claude（由 Anthropic 制造）。AI 生成的文字——困境分析、辩论交锋、年度回顾——最好被当作一份引人深思的提示，而非权威真理。我们竭力让它们既有用又公允，但它们有时会出错、会遗漏语境，或会反映出从模型继承而来的偏见。不要仅凭一次 AI 分析就做出人生抉择。
            </p>
            <p>
        ...
        <PixelWindow title="责任 · 隐私 · 法律 · 联系方式">
          <Prose>
            <p>
              <strong>责任限制。</strong> Mull 按&ldquo;现状&rdquo;提供。在法律允许的范围内，维护者对因你使用 Mull 而产生的间接、附带或后果性损害概不负责。倘若我们这一端出了灾难性的差错，我们的责任上限为你在过去十二个月内向我们支付的金额——而目前那是零英镑。
            </p>
            <p>
              <strong>隐私。</strong> 参见{' '}
              <Link
              ...
              。简短版本：我们只保留让 Mull 得以运作所需的最低限度数据，没有广告，不出售数据，你随时可以下载或删除一切。
            </p>
            <p>
              <strong>适用法律。</strong> 本条款受英格兰法律管辖。争议交由英格兰及威尔士的法院处理，除非你是消费者——在这种情况下，你当地的消费者保护法亦同时适用。
            </p>
            <p>
              <strong>联系方式。</strong>{' '}
              <a
