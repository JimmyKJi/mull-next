...
export default function MatchClient(props: Props) {
  ...
  const canCallVerdict =
    status === 'active' && userTurns >= 2 && turns[turns.length - 1]?.speaker === 'opponent';
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {canCallVerdict && (
                <button type="button" onClick={callJudge} disabled={judging} style={btnGhost}>
                  {judging
                    ? t('arena.match_judging', props.locale)
                    : t('arena.match_call_verdict', props.locale)}
                </button>
              )}
              <button
                type="button"
              ...
              >
                {submitting
                  ? t('arena.match_thinking', props.locale)
                  : t('arena.match_submit', props.locale)}
              </button>
            </div>
          </div>
          {error && (
    ...
    <div style={{ marginBottom: 12 }}>
      ...
      <div
        style={{
          ...
          border: '3px solid var(--color-ink)',
          boxShadow: isUser ? '3px 3px 0 0 #2F5D5C' : '3px 3px 0 0 var(--color-acc)',
          fontFamily: serif,
          fontSize: 16,
          color: 'var(--color-ink)',
          lineHeight: 1.6,
