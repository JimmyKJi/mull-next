...
import { type Crucible, type CrucibleReport, CRUCIBLE_KEY } from '@/lib/crucible';
...
export default function CrucibleClient({
  ...
  function commitToday() {
    ...
    persistReports(next);
    setCommitted(true);
    emitFeatureEvent('crucible', `Committed to Crucible · ${today.category}`, 1);
  }
  ...
  const streak = (() => {
    const set = new Set(
      reports.filter((r) => r.status === 'kept' || r.status === 'tried').map((r) => r.dateKey),
    );
    let count = 0;
    ...
    <div className="space-y-5">
      ...
      {!yesterdayReported && (
        ...
        >
          ...
          <p className="mt-2 text-[16px] leading-[1.55] text-ink" style={{ fontFamily: serif }}>
            {yesterdayL.prompt}
          </p>
          <textarea
            value={yesterdayNote}
      ...
      >
        ...
        <h2 className="mt-4 text-[22px] leading-[1.35] text-ink" style={{ fontFamily: serif }}>
          {todayL.prompt}
        </h2>
        <p
