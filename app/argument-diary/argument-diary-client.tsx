...
import { saveAnthologyEntry } from '@/lib/anthology';
...
export default function ArgumentDiaryClient({ locale = 'en' }: { locale?: Locale }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(e.error || t('argdiary.err_request', locale, { status: res.status }));
      }
      ...
      >
        ...
        <button
          ...
          style={{
            fontFamily: pixel,
            textTransform: 'uppercase',
            background: submitting || !account.trim() ? 'var(--color-line)' : '#F8C75E',
            boxShadow: submitting || !account.trim() ? 'none' : '4px 4px 0 0 #8C3717',
            cursor: submitting || !account.trim() ? 'default' : 'pointer',
          }}
        >
          {submitting ? t('argdiary.analyzing', locale) : t('argdiary.analyze', locale)}
        </button>
      </div>
        ...
        >
          ...
          <ul className="mt-3 space-y-2">
            {history
              ...
              .map((h) => (
                <li key={h.id}>
                  <details className="border-l-2 border-acc-deep bg-[#FBF6E8] px-3 py-2">
                    <summary
                      className="cursor-pointer text-[13px] text-ink"
                      style={{ fontFamily: serif }}
                    >
                      {new Date(h.ts).toLocaleDateString(locale === 'zh' ? 'zh-CN' : undefined)} —{' '}
...
function AnalysisCard({ entry, locale }: { entry: DiaryEntry; locale: Locale }) {
  return (
    <div className="space-y-3">
      <Section title={t('argdiary.section_steelman', locale)} accent="#2F5D5C">
        <p className="text-[15px] leading-[1.65] text-ink" style={{ fontFamily: serif }}>
          ...
          {entry.analysis.kindred?.map((k, i) => (
            <li key={i}>
              <div className="flex items-baseline justify-between gap-2">
                ...
                <SaveTakeButton philosopher={k.philosopher} take={k.take} locale={locale} />
              </div>
              <p
                className="mt-1 text-[14.5px] leading-[1.55] text-ink"
                style={{ fontFamily: serif }}
