...
export default function YearView({ locale }: { locale: Locale }) {
  ...
  const grid: number[][] = Array.from({ length: 12 }, () => Array(SKILLS.length).fill(0));
  ...
  const activeDays = new Set(thisYearsEvents.map((e) => new Date(e.ts).toISOString().slice(0, 10)))
    .size;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat
          label={t('yr.stat_moves', locale)}
          value={String(thisYearsEvents.length)}
          color="#B8862F"
        />
        <Stat label={t('yr.stat_active_days', locale)} value={String(activeDays)} color="#2F5D5C" />
        <Stat
          label={t('yr.stat_saved', locale)}
          value={String(thisYearsAnthology.length)}
          color="#8C3717"
        />
        <Stat
                  ...
                  <tr key={s}>
                    ...
                    {MONTH_LABELS.map((_, mi) => {
                      ...
                      return (
                        <td
                          key={mi}
                          title={t('yr.cell_tooltip', locale, {
                            skill: meta.name,
                        ...
                        >
                          <div
                            style={{
                              width: '100%',
                              height: '100%',
                              background: v === 0 ? '#F5EFDC' : meta.color,
                              opacity: v === 0 ? 1 : Math.max(0.25, intensity),
                              border:
                                mi === new Date().getUTCMonth()
...
function Stat({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    ...
    >
      <div className="text-[22px] leading-none text-ink" style={{ fontFamily: pixel }}>
        {value}
      </div>
      <div
