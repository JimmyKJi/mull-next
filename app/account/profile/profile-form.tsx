    ...
    >
      <div>
        <label htmlFor="handle" style={labelStyle}>
          {t('profile.handle_label', locale)}
        </label>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontFamily: serif, fontSize: 18, color: 'var(--color-acc-deep)' }}>
            mull.world/u/
          </span>
          <input
            id="handle"
      ...
      <div>
        <label htmlFor="displayName" style={labelStyle}>
          {t('profile.display_name_label', locale)}
        </label>
        <input
          id="displayName"
          type="text"
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <Checkbox
            label={t('profile.show_archetype', locale)}
            checked={showArchetype}
            onChange={setShowArchetype}
          />
          <Checkbox
            label={t('profile.show_dimensions', locale)}
            checked={showDimensions}
            onChange={setShowDimensions}
          />
          <Checkbox label={t('profile.show_map', locale)} checked={showMap} onChange={setShowMap} />
          <Checkbox
            label={t('profile.show_streak', locale)}
            checked={showStreak}
      ...
      >
        <strong style={{ color: 'var(--color-ink)' }}>{t('profile.never_visible', locale)}</strong>{' '}
        {t('profile.never_visible_body', locale, { email: userEmail })}
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        >
          {submitting
            ...
            : isExisting
              ? t('profile.update', locale)
              : t('profile.publish', locale)}
        </button>
        {isExisting && (
          <button
            type="button"
      ...
      {savedUrl && (
        ...
        >
          ...
          <Link
            href={savedUrl}
            style={{ color: '#2F5D5C', textDecoration: 'underline' }}
            target="_blank"
          >
            mull.world{savedUrl}
          </Link>
