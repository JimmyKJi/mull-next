...
export function ResearchConsentGate({ children, isLoggedIn = false, locale: localeProp }: Props) {
    ...
    >
      <div style={{ maxWidth: 580, width: '100%' }}>
        ...
        >
          ...
          >
            <p style={{ margin: '0 0 12px' }}>{emph(t('consent.gate_p1', locale))}</p>
            <p style={{ margin: '0 0 12px' }}>{emph(t('consent.gate_p2', locale))}</p>
...
function DemographicsStep({ onDone, locale }: { onDone: () => void; locale: Locale }) {
  return (
    <div
      style={{
        background: '#26201A',
