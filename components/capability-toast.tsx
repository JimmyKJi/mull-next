  ...
  useEffect(() => {
    ...
    function handler() {
      ...
      const newToasts: Toast[] = newOnes.map((e) => {
        ...
        const beforeXp = aggregateXp(events.filter((x) => x.skill === e.skill && x.ts < e.ts))[
          e.skill
        ];
        const afterXp = aggregateXp(events.filter((x) => x.skill === e.skill && x.ts <= e.ts))[
          e.skill
        ];
      ...
      {toasts.slice(-3).map((t) => {
        ...
        return (
          ...
          >
            ...
            >
              {t.event.label ??
                tr('uic.toast_practice_in', locale, { skill: meta.name.toLowerCase() })}
            </div>
          </div>
        );
      })}
