...
function buildCartographer(): SpriteGrid {
  ...
  ring(g, 16, 16, 13, "+", 2);
  ring(g, 16, 16, 11, "^", 1);
  fillPoly(g, [[16, 4], [18, 15], [16, 15], [14, 15]], "^");
  fillPoly(g, [[16, 28], [14, 17], [16, 17], [18, 17]], "+");
  fillPoly(g, [[28, 16], [17, 14], [17, 16], [17, 18]], "*");
  fillPoly(g, [[4, 16], [15, 18], [15, 16], [15, 14]], "^");
  fillCircle(g, 16, 16, 2, "#");
  setPx(g, 16, 16, "^");
  setPx(g, 16, 3, "#"); setPx(g, 16, 29, "#"); setPx(g, 3, 16, "#"); setPx(g, 29, 16, "#");
  outline(g, "#");
  ring(g, 16, 16, 14, "#", 1);
  return toRows(g);
}
...
function buildKeel(): SpriteGrid {
  const g = makeGrid();
  ring(g, 16, 6, 3, "*", 2);
  fillRect(g, 15, 8, 3, 17, "*"); fillRect(g, 17, 8, 1, 17, "+"); fillRect(g, 15, 8, 1, 17, "^");
  fillRect(g, 9, 11, 14, 2, "*"); fillRect(g, 9, 12, 14, 1, "+"); fillRect(g, 9, 11, 14, 1, "^");
  for (let x = 7; x <= 25; x++) {
    const u = 81 - (x - 16) ** 2;
    if (u < 0) continue;
    const y = 17 + Math.sqrt(u);
    setPx(g, x, Math.round(y), "*"); setPx(g, x, Math.round(y) - 1, "*"); setPx(g, x, Math.round(y) + 1, "+");
  }
  fillPoly(g, [[7, 22], [4, 18], [10, 16]], "*");
  fillPoly(g, [[25, 22], [28, 18], [22, 16]], "*");
  outline(g, "#");
  return toRows(g);
}
...
function buildThreshold(): SpriteGrid {
  const g = makeGrid();
  fillRect(g, 8, 13, 16, 15, "*"); fillCircle(g, 16, 13, 8, "*");
  fillRect(g, 8, 13, 2, 15, "^"); fillRect(g, 22, 13, 2, 15, "+");
  fillRect(g, 12, 15, 8, 13, "~"); fillCircle(g, 16, 15, 4, "~");
  fillCircle(g, 16, 19, 2, "^");
  fillRect(g, 6, 28, 20, 3, "+"); fillRect(g, 6, 28, 20, 1, "*");
  outline(g, "#");
  return toRows(g);
}
...
function buildPilgrim(): SpriteGrid {
  const g = makeGrid();
  fillRect(g, 21, 4, 2, 25, "+"); setPx(g, 21, 4, "^"); fillCircle(g, 22, 4, 2, "*");
  fillCircle(g, 13, 9, 3.5, "~");
  fillPoly(g, [[13, 12], [7, 29], [19, 29]], "*"); fillRect(g, 7, 28, 12, 2, "*");
  fillPoly(g, [[13, 12], [7, 29], [9, 29]], "^"); fillPoly(g, [[13, 12], [17, 29], [19, 29]], "+");
...
function buildTouchstone(): SpriteGrid {
  const g = makeGrid();
  fillPoly(g, [[10, 11], [22, 11], [26, 17], [16, 28], [6, 17]], "*");
  fillPoly(g, [[10, 11], [22, 11], [19, 17], [13, 17]], "^");
  fillPoly(g, [[6, 17], [13, 17], [16, 28]], "+"); fillPoly(g, [[16, 28], [19, 17], [26, 17]], "+");
  drawLine(g, 6, 17, 26, 17, "+"); drawLine(g, 13, 17, 16, 28, "+"); drawLine(g, 19, 17, 16, 28, "+");
  drawLine(g, 10, 11, 13, 17, "+"); drawLine(g, 22, 11, 19, 17, "+");
  setPx(g, 12, 13, "~"); setPx(g, 11, 14, "~");
  outline(g, "#");
  return toRows(g);
}
...
function buildHearth(): SpriteGrid {
  const g = makeGrid();
  fillPoly(g, [[16, 4], [21, 12], [22, 20], [16, 26], [10, 20], [11, 13], [14, 10]], "*");
  fillCircle(g, 16, 20, 6, "*");
  fillCircle(g, 16, 21, 4, "^"); fillCircle(g, 16, 22, 2, "~");
  fillRect(g, 8, 26, 16, 4, "+"); fillRect(g, 8, 26, 16, 1, "*");
  outline(g, "#");
  setPx(g, 12, 6, "~"); setPx(g, 20, 7, "~");
  return toRows(g);
}
...
function buildForge(): SpriteGrid {
  const g = makeGrid();
  fillRect(g, 6, 24, 20, 4, "*"); fillRect(g, 6, 27, 20, 1, "+");
  fillRect(g, 12, 20, 8, 5, "*"); fillRect(g, 18, 20, 2, 5, "+");
  fillPoly(g, [[6, 14], [24, 14], [24, 19], [12, 19]], "*"); fillPoly(g, [[6, 14], [12, 19], [2, 17]], "*");
  fillRect(g, 6, 14, 18, 1, "^"); fillRect(g, 6, 18, 18, 1, "+");
  fillRect(g, 12, 11, 7, 3, "^"); fillRect(g, 13, 11, 5, 1, "~");
  outline(g, "#");
  setPx(g, 12, 9, "^"); setPx(g, 16, 7, "~"); setPx(g, 20, 9, "^"); setPx(g, 22, 6, "~"); setPx(g, 15, 5, "~");
  return toRows(g);
}
...
function buildHammer(): SpriteGrid {
  const g = makeGrid();
  fillRect(g, 15, 12, 3, 17, "*"); fillRect(g, 17, 12, 1, 17, "+"); fillRect(g, 15, 12, 1, 17, "^");
  fillRect(g, 8, 6, 16, 9, "*"); fillRect(g, 8, 6, 16, 1, "^"); fillRect(g, 8, 14, 16, 1, "+");
  fillRect(g, 8, 6, 3, 9, "+"); fillRect(g, 21, 6, 3, 9, "+"); fillRect(g, 8, 6, 1, 9, "^");
  fillRect(g, 13, 8, 6, 5, "+");
  outline(g, "#");
  return toRows(g);
}
...
function buildGarden(): SpriteGrid {
  const g = makeGrid();
  fillPoly(g, [[16, 5], [12, 15], [16, 19]], "^"); fillPoly(g, [[16, 5], [20, 15], [16, 19]], "*");
  fillPoly(g, [[10, 9], [6, 17], [15, 16]], "*"); fillPoly(g, [[22, 9], [26, 17], [17, 16]], "*");
  fillRect(g, 15, 15, 2, 7, "+");
  fillPoly(g, [[9, 22], [23, 22], [20, 30], [12, 30]], "+"); fillRect(g, 8, 21, 16, 2, "*");
  fillRect(g, 9, 22, 1, 8, "^");
  outline(g, "#");
  return toRows(g);
}
...
function buildLighthouse(): SpriteGrid {
  const g = makeGrid();
  fillPoly(g, [[12, 12], [20, 12], [22, 28], [10, 28]], "*");
  fillRect(g, 11, 16, 11, 2, "+"); fillRect(g, 10, 22, 12, 2, "+");
  fillPoly(g, [[12, 12], [14, 12], [12, 28], [10, 28]], "^");
  fillRect(g, 12, 7, 8, 5, "~"); fillCircle(g, 16, 9, 2, "^");
  fillPoly(g, [[11, 7], [21, 7], [16, 3]], "+");
  fillRect(g, 8, 28, 16, 2, "+"); fillRect(g, 8, 28, 16, 1, "*");
  outline(g, "#");
  setPx(g, 24, 8, "^"); setPx(g, 26, 7, "~"); setPx(g, 8, 8, "^"); setPx(g, 6, 7, "~");
  return toRows(g);
}
...
export function ArchetypeSprite({
  ...
  for (let y = 0; y < n; y++) {
    ...
    while (x < row.length) {
      ...
      rects.push(
        <rect key={`${x}-${y}`} x={x} y={y} width={run} height={1} fill={f} />,
      );
      x += run;
    }
  }
